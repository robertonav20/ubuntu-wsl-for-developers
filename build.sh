echo "Build Image ubuntu-wsl:1.4.0"
docker build --tag ubuntu-wsl:1.4.0 --file Dockerfile .

echo "Publish Image ubuntu-wsl:1.4.0"
...
echo "Exporting Container ubuntu-wsl:1.4.0 as archive"
docker stop ubuntu-wsl || true && docker rm ubuntu-wsl || true
docker run --name ubuntu-wsl -it ubuntu-wsl:1.4.0 bash -C exit
docker export --output ubuntu-wsl-1.4.0.tar ubuntu-wsl
